...
export const TALENT_ROUTE: Route = {
  path: 'freelancer',
  ...
  loadChildren: () => [
    ...
    },
    {
      path: 'profile',
      pathMatch: 'full',
      loadComponent: () => import('./profile.page').then(m => m.TalentProfilePageComponent),
      resolve: {
        profile: async () => {
          const api = inject(TalentApi);
    ...
    },
    {
      path: 'profile/edit',
      pathMatch: 'full',
      loadComponent: () => import('./profile-edit.page').then(m => m.TalentProfileEditPageComponent)
    },
    {
