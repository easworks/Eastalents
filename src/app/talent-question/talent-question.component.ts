...
import {
  TalentProfile,
  TalentProfileSteps,
} from '../_models/taltent-profile-steps';
...
@Component({
  ...
  styleUrls: ['./talent-question.component.css'],
})
export class TalentQuestionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.idToNavigate) {
      ...
      this.showProfessionalCodingExperience =
        this.idToNavigate === 8 || this.idToNavigate === 7 ? true : false;
  ...
  initializeValue() {
    ...
    this.talentQuestionData.unshift({
      question: { first: 'Select your Primary ', second: 'role' },
      id: 6,
      type: 'mixed',
      option: this.completeJobs,
    });
    this.talentQuestionData.unshift({
      question: {
        first: 'Select the ',
        second: 'system phase you have worked',
      },
      id: 5,
      type: 'mixed',
      option: this.completePhase,
    });
    this.talentQuestionData.unshift({
      question: {
        first: 'Select your Primary ',
        second: 'software experience',
      },
      id: 4,
      type: 'mixed',
      option: this.completeProduct,
    });
    this.talentQuestionData.unshift({
      question: { first: 'Select the ', second: 'module to have specialized' },
      id: 3,
      type: 'mixed',
      option: this.datathree,
    });
    this.talentQuestionData.unshift({
      question: {
        first: 'Select Your Primary Enterprise Application',
        second: '',
      },
      id: 2,
      type: 'mixed',
      option: this.dataKeys,
    });
    this.talentQuestionData.unshift({
  ...
  getDynamicData() {
    this.httpService.get('talentProfile/getTalentProfileSteps').subscribe(
  ...
  getData() {
    return this.http
      .get('/assets/talentquestion.json')
      .subscribe((res: any) => {
  ...
  onSelectCountry(opt: any) {
    this.talentQuestionData[1].country = this.countries.find(
  ...
  onAddExperience() {
    this.talentQuestionData[this.currentPage].experience.push({
      role: '',
      startDuration: '',
      endDuration: '',
      skills: '',
  ...
  onAddEducation() {
    this.talentQuestionData[this.currentPage].education.push({
      degree: '',
      specialization: '',
      startDuration: '',
      endDuration: '',
  ...
  uploadPdf() {
    ...
    uploadData.append(
      'userId',
      this.sessionService.getLocalStorageCredentials()._id
    );
    this.httpService
      .post('talentProfile/setTalentProfileResume', uploadData)
      .subscribe(
        (response: ApiResponse<any>) => {
          if (response.status) {
  ...
  uploadVideo() {
    ...
    this.httpService
      .post('talentProfile/setTalentProfileVideo', uploadData)
      .subscribe(
        (response: ApiResponse<any>) => {
          if (response.status) {
  ...
  uploadImage() {
    ...
    this.httpService
      .post('talentProfile/setTalentProfilePhoto', uploadData)
      .subscribe(
        (response: ApiResponse<any>) => {
          if (response.status) {
            this.toaster.success(`${response.message}`);
  ...
  onUpdateProfile() {
    ...
    this.httpService.post('talentProfile/updateTalentProfile', data).subscribe(
      (response: ApiResponse<any>) => {
        if (response.status) {
  ...
  selectOption(opt: any, type?: any) {
    ...
    if (this.currentPage === 1) {
      ...
      if (opt.subType === 'select') {
        this.talentQuestionData[this.currentPage].data.forEach(
          (element: any) => {
            if (element.type === 'select') {
              element.option.forEach((optionData: any) => {
                if (
                  opt.selected === true &&
                  opt.firstPart != optionData.firstPart
                ) {
        ...
        if (
          opt.selected === true &&
          opt.firstPart === 'Actively Looking For Jobs'
        ) {
        ...
        this.talentQuestionData[this.currentPage].data.forEach(
          (element: any) => {
            if (element.type === 'multiselect') {
              element.option.forEach((optionData: any) => {
                if (
                  opt.firstPart.toLowerCase() ===
                  'Any of the above'.toLowerCase()
                ) {
      ...
      if (opt.type === 'startWorkingWithEasyWork') {
        opt.selected = !opt.selected;
        this.talentQuestionData[
          this.currentPage
        ].startWorkingWithEasyWork.forEach((optionData: any) => {
          if (
            opt.selected === true &&
            opt.value.toLowerCase() != optionData.value.toLowerCase()
          ) {
            optionData.disable = true;
          } else if (opt.selected === false) {
            optionData.disable = false;
        ...
        this.talentQuestionData[this.currentPage].data.preferredPLM =
          this.preferredPLM;
      } else if (type === 'currentPLM') {
        this.currentPLM = opt;
        this.talentQuestionData[this.currentPage].data.currentPLM =
          this.currentPLM;
      }
  ...
  upDateYearInQues2(value: any, completeData: any) {
    this.talentQuestionData[this.currentPage].option.forEach((element: any) => {
      if (
        completeData.value.toLowerCase() === element.value.toLowerCase() &&
        element.selected === true
      ) {
  ...
  enableDisableNextButton() {
    ...
    if (this.currentPage === 1) {
      if (
        this.talentQuestionData[this.currentPage].profileSummary != '' &&
        this.talentQuestionData[this.currentPage].profileSummary.length > 200 &&
        this.selectedCity &&
      ...
      if (this.talentQuestionData[this.currentPage].startFreelancing) {
        if (
          ...
          this.talentQuestionData[this.currentPage].option[1].selected === true
        ) {
          this.disableNextButton = false;
        }
      }
  ...
  getButtonEnableForElev() {
    ...
    if (
      opt.data.desiredHourlyrate &&
      opt.data.targetAnnualSalary &&
      opt.data.monthlyRate &&
      opt.data.selectedPLM &&
      opt.data.weeklyHourlyRate &&
      this.getButtonEnableForMultiSelect(opt.startWorkingWithEasyWork)
    ) {
      return false;
    } else {
      return true;
  ...
  goToMyProfile() {
    ...
    this.httpService.post('talentProfile/createTalentProfile', data).subscribe(
      (response: ApiResponse<any>) => {
        if (response.status) {
          this.toaster.success(`${response.message}`);
          this.router.navigate(['/Talent-Profile-Edit']);
  ...
  getanswerData() {
    ...
    this.talentQuestionData.forEach((data: any) => {
      if (data.id === 1) {
        ...
        answer.push({
          id: data.id,
          startFreelancing: data.startFreelancing,
          startWorkingProfessionally: data.startWorkingProfessionally,
          selectedYesNo: '',
        });
        data.option.forEach((opt: any) => {
        ...
        data.option.forEach((opt: any) => {
          if (opt.selected === true) {
            answer[data.id].option.push({
              value: opt.value,
              skill: opt.skill,
              noOfYear: opt.noOfYear,
            });
          }
        });
